...
import { ProjectActionTypes, GetProjects, GetProject, GetProjectsSuccess, GetProjectSuccess } from '../actions/project.actions';
  ...
  GetProjects: Observable<any> = this.actions$.pipe(
    ...
    switchMap(() =>
      this.projectService.getAllProjects().pipe(
        map(data => new GetProjectsSuccess(data),
  ...
  GetProject: Observable<any> = this.actions$.pipe(
    ...
    switchMap(id => (
      this.projectService.getProjectById(id).pipe(
        map(res => new GetProjectSuccess(res)),
        catchError((err) => of(new GetErrors({ message: err })))
      )
    ))
  )
